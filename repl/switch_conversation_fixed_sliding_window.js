...
export async function switch_sliding_window_conversation(conv_id) {
	let user_response;
	let model_response;
	...
	let conversation_history = fs.readFileSync(
	...
	while (true) {
		...
		conversation_history = fs.readFileSync(`${chat_save_dir}/${fileName}`);
		parsed_conversation_history = JSON.parse(conversation_history.toString());
