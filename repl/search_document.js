...
import { search_documents, scroll_document } from "../database.js";
...
export async function search_document(semantic_search) {
	...
	if (current_conversation_id) {
		const search_result = await search_documents(embedding, current_conversation_id);
		console.log("search result -", search_result);
		return;
	}
	console.log("please select a conversation id");
}
