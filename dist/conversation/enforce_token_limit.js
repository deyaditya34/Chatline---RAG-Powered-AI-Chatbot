...
export async function enforceTokenLimit(parsedConversationHistoryUser, parsedConversationHistoryModel, userPromptEmbedding, userResponse, convName) {
    ...
    if (tokenConsumed.totalTokens >= conversationTokenLimit) {
        ...
        const sanitizedAiSummarizePrompt = sanitizeConversation(aiPrompts.default.summarizeUserConvPrompt, "user");
        parsedConversationHistoryModel.contents.push(sanitizedAiSummarizePrompt);
        while (true) {
            let [modelResponse] = await generateContent(parsedConversationHistoryModel.contents);
...
//# sourceMappingURL=enforce_token_limit.js.map