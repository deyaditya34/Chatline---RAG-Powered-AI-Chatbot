...
export async function embedDocument(docPath, convName) {
    ...
    for await (const line of rl) {
        ...
        if (line.trim() === "") {
            if (paragraph.trim()) {
                ...
                await storeUploadedDoc(convName, documentDetails.documentId, documentDetails.uploadedAt, paragraph, embedding);
                paragraph = "";
            }
        }
        else {
    ...
    await storeConversationUserFile(convName, parsedConversationHistoryUser);
    printMessage(`file - '${docPath}' embedding completed.`);
}
//# sourceMappingURL=embed_document.js.map