...
export async function switchConversation(convName: string): Promise<void> {
	...
	while (true) {
		...
		if (parsedConversationHistoryUser.uploadDocuments.length > 0) {
			const pastUploadedDocumentContext = await retrieveUploadedDocuments(
				userPromptEmbedding,
				userResponse,
				convName
			);
		...
		if (parsedConversationHistoryUser.tokenLimitExceededOnce) {
			const pastConversationContext = await retrieveConversationHistory(
				userPromptEmbedding,
				userResponse,
				convName
			)
		...
		let [modelResponse, modelVersion] = await generateContent(
