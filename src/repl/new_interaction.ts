...
export async function newInteraction(convName: string): Promise<void> {
	let userResponse: string;
	let prevModelResponseId: string = "";
	...
	while (true) {
		...
		let [modelResponse, modelResponseId, modelVersion] =
			await createNewInteraction(userResponse, prevModelResponseId);
