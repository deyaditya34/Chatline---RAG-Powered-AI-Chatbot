...
import {
	sanitizeInteraction,
	sanitizeAndPrintInteraction,
	parsePrevModelResponseId
...
export async function switchInteraction(convName: string): Promise<void> {
	let userResponse;
	...
	while (true) {
		...
		let [modelResponse, modelVersion, modelResponseId] =
			await createNewInteraction(userResponse, prevModelResponseId, prevModelVersion);
