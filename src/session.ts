...
export function setConversationMode(convMode: string): void {
	mode = convMode;
}

export function setCurrentConversationIdFromConvName(convName: string): void {
...
export function setCurrentConversationIdFromConvId(convId: string): void {
	if (!convId) {
	...
	if (
		fileNo > conversationList.length - 1 ||
		!Number(convId) ||
...
export function setCurrentConversationIdFromInteractionName(interactionName: string): void {
...
export function setCurrentConversationIdFromInteractionId(interactionId: string): void {
	if (!interactionId) {
		console.log("usage: /switch <id>");
		return;
	}

	const interactionList = fs.readdirSync(
		`${interactionsDir}`
	);
	...
	if (
		fileNo > interactionList.length - 1 || 
